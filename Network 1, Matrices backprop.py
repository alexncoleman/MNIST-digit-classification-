...
# -*- coding: utf-8 -*-
"""
Created on Sun Sep  4 15:17:16 2022

@author: alexcoleman

Code from: Michael A. Nielsen, "Neural Networks and Deep Learning", Determination Press, 2015
I have just changed the backpropagation algorithm to use matrix rather than single vectors
...
class Network(object):
    ...
    def update_mini_batch(self, mini_batch, eta, mini_batch_size):
        ...
        X = []#put all inputs/outputs from minibatch in matrix X/Y
        Y = []
        for x,y in mini_batch:
            X.append(x)
            Y.append(y)
        X = np.squeeze(np.array(X)).transpose() #X is a matrix with 784 rows and 10 columns now
        Y = np.squeeze(np.array(Y)).transpose()
    
        nabla_b, nabla_w = self.backprop(X,Y, mini_batch_size) #uses func. backprop (defined below)
    ...
    def backprop(self, x, y, mini_batch_size):
       #backprop given X and Y where X is first layer activations matrix and Y is output layer values matrix
       ...
       #'nabla_b' and 'nabla_w' are layer-by-layer lists of numpy arrays, similar to self.biases and self.weights
        ...
        for b,w in zip(self.biases, self.weights):
            z = np.dot(w,activation) + b #use broadcasting to add through the elements
            ...
            with the first array of weights (single activation value multiplied to all weights), 
            corresponding to the activation of the first neuron and the weights it has
        ...
        #backward pass
        delta = self.cost_derivative(activations[-1], y) * sigmoid_prime(zs[-1]) #uses final activations and desired output activations to find derivative of cost function (using quadratic cost function)
        nabla_b[-1] = np.reshape(np.sum(delta,1), (self.sizes[-1],1)) #how the final biases should change is just proportional to gradient of cost function
                                                          #then sums across rows to find the change in biases to add up the change calculated from 10 different input vectors
        for x in range(mini_batch_size):
            delta_M = delta[:,x] #take column of delta (so change for first input)
            active_M = activations[-2].transpose()[x,:] #take rows of the activations
            nabla_w_M = np.dot(delta_M[:,np.newaxis], active_M[np.newaxis,:]) #how weights should change depends on activations in second to last layer as well. also if differentated sigmoid wrt weights than change must be multplied by activations
                                                                              #this multiplies each input column by the activations it experiences in second to last layer
            nabla_w[-1] += nabla_w_M #add up all changes in weights corresponding to the different input vectors 
        ...
        for l in range(2, self.num_layers): #already carried out this process when l = 1 above so start from l =2
            ...
            nabla_b[-l] = np.reshape(np.sum(delta,1),(self.sizes[-l],1)) #sum across rows 
            for x in range(mini_batch_size): #for loop multiplies delta with activations for each different pass in mini batch
                delta_M = delta[:,x]
                active_M = activations[-l-1].transpose()[x,:]
                nabla_w_M = np.dot(delta_M[:,np.newaxis], active_M[np.newaxis,:])
                nabla_w[-l] += nabla_w_M
